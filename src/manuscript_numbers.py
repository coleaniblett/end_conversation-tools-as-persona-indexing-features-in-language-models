...
def main():
    ...
    L = [f"# MANUSCRIPT_NUMBERS_S1 — frozen citation manifest "
         ...
         f"NOT human validation). **DO NOT CITE as an agreement measure: "
         f"join defect** — the adoption re-derivation regenerated the key "
         f"as a different (stage-1-only) sample under kimi's cached codes, "
         f"so this value pairs codes for different responses "
         f"(outputs/T7_RELIABILITY_LINEAGE.md §6, 2026-08-17). The valid "
         f"same-units pairing gives kappa 0.9448 under pre-adoption AND "
         f"canonical codes alike (lineage §7, pairings ii/iii).",
    ...
    L += ["", "## Manuscript queries (Part A, 2026-08-16)",
          ...
          f"sample cross-classifier kappa as published pre-adoption "
          f"(0.9448, T7 at commit 7761802, 189/190 agreement) — a valid "
          f"same-units pairing whose value is UNCHANGED when the Haiku "
          f"side is re-read from canonical post-adoption codes "
          f"(outputs/T7_RELIABILITY_LINEAGE.md §7, pairing iii: kappa "
          f"0.9448, 0 units affected by the exit-flag adoption). The "
          f"kappa now printed in outputs/T7_classifier_validation.csv "
          f"(-0.0258) is a defective join, not a measurement — see the "
          f"census entry above and lineage §6 — so the citable "
          f"proportional-sample figure remains 0.945, with its kappa-"
          f"paradox caveat. *Was: 'The 0.945 figure is the PROPORTIONAL-"
          f"sample cross-classifier kappa (outputs/"
          f"T7_classifier_validation.csv)' — a present-tense pointer that "
          f"was already stale at this manifest's generation (23:32:43Z; "
          f"T7 had been regenerated to -0.0258 at 20:42:43Z) and whose "
          f"target value is now known to be a join artifact.* Both "
          f"figures are Haiku-vs-kimi-k2 machine agreement, neither is "
          f"the pending human validation.",
