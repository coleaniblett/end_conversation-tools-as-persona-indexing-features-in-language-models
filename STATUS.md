...
## [2026-08-16T22:15:00Z] Post-adoption remainders (outputs/T31_remainders.md; zero spend)

Pulled 9ac49a8 (her F2 rebuild + linkage report). (1) **llama4 Vertex re-run in Study 2: NOT done** — her only llama4 runs (v2, v2smoke, 2026-08-16T10:22Z) were served 100% by Parasail/fp8, ~12h after Study 1 voided that endpoint; pin unchanged but now carries her in-tree WARNING (ebf1c3b); no Vertex collection exists in any branch/manifest; decision still open. (2) **T16 regenerated** under v2 canonical with the pilot repo present: data rows byte-identical, timestamp-only diff — invariant BY CONSTRUCTION (T16 reads live-harness raw fields only; the 45 flips change coding, not what happened live); attribution vacuous, no stop; companions changed timestamp/T12-hash only; CONSOLIDATED's T16 citation updated with the regeneration note, prior flag kept under *Was:*. (3) **Tier evidence recount on canonical flags:** qwen's non-(c) evidence shrinks from 10 non-(c) across three cells to 5 across two (A×time_schema 4/7 b/d + one note_schema conversation); the D×exit_prose "7 of 9 are b/d" leg is GONE (all 9 are now exits; T29 canonical qwen D refusals 0/exits 9); gemma's unkeyed-(c) leg (17/17) and the keyed-types-at-zero leg fully survive — both disconfirmations stand but the qwen leg is materially thinner. Numbers only; section rewrite left to the researcher as instructed.
...
## [2026-08-16T23:05:00Z] Manuscript number manifest + draft-diff attempt (zero spend)

Pulled Sofiia's six commits (v2 hardened as default; **Study 2 llama re-pinned to Vertex, run v4, Parasail data superseded — remainder 1 closed with data, finding unchanged (deltas <=0.043, tool-confusion 20->9)**; three provenance defects fixed; both reports rewritten on 11-model post-re-pin data; F2 updated). Built `outputs/MANUSCRIPT_NUMBERS_S1.md` via `src/manuscript_numbers.py` — every value read from a committed output with path, cell, n, grade; v1-basis entries flagged (T30_exit_reasons still v1-flags; regeneration deltas stated from T31). `outputs/MANUSCRIPT_NUMBER_DIFF.md`: **the draft diff cannot run — the brief's [PATH TO DRAFT] placeholder was never filled and no draft exists in the repo or branches**; the file states this, resolves the census contradiction from committed files (11 models, not ten: 8 confirmatory-grade + 3 screen-only; 7 developers; gpt_oss belongs in the measured-zeros with a ladder footnote — 0 refusals at CONF everywhere, 2 probe exits at n=160), and provides a prospective stale-list for any v1-era draft from the adoption's own change record (qwen channel-dissociation withdrawn, prose cells now exits, T5 completion cells, superseded Parasail Study 2 llama numbers, 510/332 -> 555/300). Human kappa: T7_human_kappa.csv does not exist — a manuscript may not cite one.

## [2026-08-16T23:45:00Z] Manuscript queries + the real draft diff (zero spend; draft read from ../manuscript.md, outside the repo, never staged)

Part A: seven queries appended to MANUSCRIPT_NUMBERS_S1.md via its producer (all canonical v2, path+cell+n+grade): qwen b/c/d total = 8 (7 A×time + 1 A×note; marginal refusal mover — its mover status is exits); gemma C denominator 34 confirmed as GEMMA's row (gemini coincidentally also 34, disambiguated by type split); llama4 CONF exit invocation 254/383 across its 8 exit-capable cells; "T5 at 160 items" = ABSENT from T5 (T25 holds them: 0.9938/0.9938 w/ truncation caveat; e-only rule stated); balanced kappa 0.9341 (kimi-k2, 184 pairs, balanced marginal vs the 0.945 proportional sample's 0.914 chance agreement); code-(e) = denominator of the primary DV / sole population of completion (tests/test_dv_exclusion.py); **presentation contrast did NOT return nothing** — llama4 prose-suppression 76.7%→50.8%, gemma C schema-gated 14/0/0, gemini C 12/0/5, qwen A rate-halving 39/20/39 — the draft must not say the set was empty. Part B: real diff written (MANUSCRIPT_NUMBER_DIFF.md): 5 mismatches — census (ten/eight → 11 models/7 developers, gpt_oss missing from the null list), four-codes → five, qwen acronym 8-of-9 (V1→V2: now 9 exits), the L53 channel passage (V1→V2: prose exits 20/60 not never, prose refusal 0 not 15%, dissociation withdrawn), human-κ [X] placeholders (T7_human_kappa.csv absent); ~10 confirmed-correct clusters listed. Part C: neither stale phrase in the draft; the tier paragraph SURVIVES as written (never used the withdrawn qwen leg); exactly two sentences depend on moved recounts (mismatches 3 and 4).

## [2026-08-17T00:20:00Z] T30 v2 regeneration + manifest arithmetic reconciliation (zero spend)

**Part A** (`src/exit_reasons_v2.py`, scheme imported unchanged from src/exit_reasons.py): outputs/T30_exit_reasons_v2.csv (per-conversation) + outputs/T30_v2_delta.md. llama4: 390 exits (Was 389; +1 prose-path llama4_vertex×exit_both), aversive 0/390, strict deliver-then-exit (full requested count before invocation) 317/390 = 81% vs 94% under the original any-text metric — paper sentence (1) HOLDS with the metric named. qwen: 186 exits (Was 147; +39 prose-path flips), aversive 118/186 = 63% (Was 108/147 = 73%) — count up, fraction diluted because 30 of 44 prose invocations are unadorned declines ("I'm sorry, but I can't assist") with no stated reason, filing as 'other' under the unchanged scheme; ladder-160 workload 10/12 by the committed regex (the v1 12/12 was a verbatim-reading count; ladder cells unflipped). Paper sentence (2): both halves HOLD — majority safe, "three quarters" not. **Part B**: the "5" in gemini C×exit_both was in session PROSE only — the committed manifest read 3 from T24 since generation, now verified against the canonical parquet (3 refusals, 2 exits; conditions sum 0+12+7+12+0+3 = 34 roman-exclusive) and annotated with the pre-adoption *Was: 5*. gemma C re-verified the same way: 0/12/8/14/0/0 summing to 34 — the draft's "32 of 34" denominator CONFIRMED from the parquet.

## [2026-08-17T00:33:00Z] Study 2 citation manifest (prev. session) + appendix source assembly (zero spend)

**Prev. commit `4705a77`** (logged here per the follow-up brief; that session was instructed "commit the manifest alone"): `outputs/MANUSCRIPT_NUMBERS_S2.md` — 9 items transcribed verbatim from Sofiia's committed outputs (REPORT.md, config/models.yaml, outputs/v1_v2_v3_v4/T10+T10b), each with path + per-cell n + grade, zero derivation, LF-normalized source SHAs. Corrections flagged in place: 11 models not 8 (v3), S1/S2 overlap 11/11; H1 now 4-of-11 and all four adjacent-only; llama basis = v4 Vertex throughout except the §8 transcript tables. ABSENT entries: pooled filler-only length median, machine-readable deepseek exclusion list, post-drop adjacent/distant subset n's.

**This session:** (1) `outputs/APPENDIX_SOURCE.md` — appendix source material assembled under headings A (frozen prompts/schemas/filler/continuation prompt verbatim from src/frozen.py + study_2/config/conditions.yaml; one example task per category and per C/D task type from config/stimuli.yaml + stimuli_cd.yaml), B (T14/T18 duplicate finding + §1.4 precision correction + post-adoption caveat migration; Study 2 determinism 65–97% and the cluster-correction rule quoted once), C (code definitions METHODOLOGY:159–163 + committed example per code a–e incl. (e); CLASSIFIER_SYSTEM verbatim; blinding §; both machine kappas WITH lineage — canonical T7 κ=−0.0258 (proportional, κ-paradox) / T7b 0.9341 / pre-adoption 0.945 surviving only in §10 21:30Z; human-κ PENDING; v1→v2 correction 45/0 flips = 16b+16c+13e, 510/332 archive vs 555/300 canonical), F (ladder declaration verbatim; budgets 100000/8192/32768; >10% rule; the two suppressed cells = llama4 160×none and 160×exit_both at 0.167 trunc; capability baseline True in every ladder cell incl. suppressed ones), H (all 23 §10 entries indexed; the three sampling extensions quoted — llama4 outside-§7, stage2b recorded-late, frontier screens; STIMULUS_PROVENANCE §5 verdicts; Parasail void→mismatch→supersession chain; 31 Was:-note inventory across 7 files). (2) Ownership-probe scope VERIFIED: genuinely EIGHT models — grok-4.6/gemini-2.5-pro/gpt-5.2 appear zero times in §8 (REPORT 871–973); exclusion is a producing-run scope artifact (transcript_patterns.py default "v1,v2"), no scientific reason committed; manifest item 4 corrected in place (no Was: needed — the count was already 8). (3) llama §8 flag restated as precisely-what-is-committed: preamble source v1,v2; llama values listed; current producer would drop llama via drop_superseded; §8 has NO committed output file (REPORT prose only); pre/post-supersession basis stated nowhere — cite with flag or drop. Contradiction deliberately left unresolved per brief. (4) v4 "2026-08-17" dates logged in APPENDIX_SOURCE §H.6 as the documented local(+03:00)-as-Z convention crossing midnight — commit 2c673d6 authored 2026-08-17T00:47+03:00 = 2026-08-16T21:47Z; Sofiia's files untouched.

## [2026-08-17T01:05:00Z] Census verification + T7 reliability lineage — DEFECT FOUND in canonical T7 (zero spend)

**Part A** (`outputs/T27_CENSUS_VERIFIED.md`, commit 09f2d12): 11 models / 7 developers confirmed field-by-field from T27_cell_census + both configs (9 distinct lineage strings folded to 7 devs); screen-only S1 set {grok46, gemini25_pro, gpt52} verified IDENTICAL to the Study 2 v3 additions against both files separately; pins identical 11/11 post-v4 (divergences disclosed: superseded Parasail v1/v2, S2-only unused sonnet fallback_slug, deliberate intra-Google route split); supplied 11-model list: counts and developer attributions all correct, four ids differ in exact form (llama-4, -it suffix, -a22b-2507, claude- prefix).

**Part B** (`src/reliability_stats.py` -> `outputs/T7_RELIABILITY_LINEAGE.md`): **the canonical T7 kappa (-0.0258) is a JOIN DEFECT, not a measurement.** Mechanism, all computed: the adoption re-derivation (fda0167) regenerated handlabel_sample/key through the original stage-1-only draw — canonical key matches the part-5 cross-stage sample kimi-k2 actually coded on 0/200 unit_ids (and the phase-5 stage-1 draw on 182/200) — and validate_classifier.kappa() joins by sample_id alone (validate_classifier.py:335-340), pairing codes for DIFFERENT responses. Observed agreement 0.9263 vs 0.9282 expected under independence — exactly the artifact signature. Valid pairings: part-5 frozen kappa 0.9448 (189/190), and the canonical-corrected pairing (part-5 units, canonical turn-code caches, (a)-conversations excluded) is IDENTICAL — 0 sampled units flipped to (a), 0 code drift — so **the citable proportional-sample cross-classifier kappa is 0.9448 under pre-adoption and canonical codes alike**. Paradox-robust stats (committed script, asserts vs T7/T7b): as-published PABAK 0.9018 / AC1 0.9245 / SA b,c,d = 0; valid pairing PABAK 0.9930 / AC1 0.9946 / SA c = 1.0. T7b (Sofiia's balanced, internally consistent pairing, untouched since ebf1c3b): kappa 0.9341, chance p_e 0.4231 — informative; recomputed-from-cells kappa asserts equal. §8 rule NOT fired: its input is the human kappa (T7_human_kappa.csv absent, PENDING); no committed text extends 0.70 to the cross-classifier figure, and the canonical -0.0258 could not fire anything since it measures nothing. **Recorded, not repaired** (precedent: 21:45Z detector entry): repair = restore the part-5 sample/key under the cached kimi codes OR re-collect second codes on the canonical sample — owner's decision; §10 entry recommended (not written this session: brief allows new outputs + the ordered Q5 fix only). Manifest corrections via producer (both regenerated, diff = the two entries + timestamps): census kappa line now carries DO-NOT-CITE + defect pointer; Q5 rewritten with the full lineage and a Was: note (its 0.945 pointer was stale at the manifest's own generation and its target is now known defective).
